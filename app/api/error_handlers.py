...
def register_error_handlers(app: FastAPI) -> None:
    ...
    async def _snap(_: Request, exc: appx.SnapshotNotFoundError) -> JSONResponse:
        return JSONResponse(
            status_code=404,
            content={"error": "snapshot_not_found", "code": appx.SnapshotNotFoundError.code, "detail": str(exc)},
        )
    ...
    async def _topic(_: Request, exc: appx.TopicNotFoundError) -> JSONResponse:
        return JSONResponse(
            status_code=404,
            content={"error": "topic_not_found", "code": appx.TopicNotFoundError.code, "detail": str(exc)},
        )
    ...
    async def _sess(_: Request, exc: appx.SessionNotFoundError) -> JSONResponse:
        return JSONResponse(
            status_code=404,
            content={"error": "session_not_found", "code": appx.SessionNotFoundError.code, "detail": str(exc)},
        )
    ...
    async def _bad(_: Request, exc: appx.InvalidRequestError) -> JSONResponse:
        return JSONResponse(
            status_code=400,
            content={"error": "invalid_request", "code": appx.InvalidRequestError.code, "detail": str(exc)},
        )
    ...
    async def _phase(_: Request, exc: appx.PhaseConflictError) -> JSONResponse:
        return JSONResponse(
            status_code=409,
            content={"error": "phase_conflict", "code": appx.PhaseConflictError.code, "detail": str(exc)},
        )
